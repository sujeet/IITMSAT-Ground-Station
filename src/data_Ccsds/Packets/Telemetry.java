...
public class Telemetry extends CcsdsPacket
{
	...
	public static TelemetrySettings getEffectiveSettings() {
		if(Settings != null)
			return Settings;
	...
	public Telemetry(int applicationProcessId, byte serviceType, byte serviceSubtype)
	{
		setApplicationProcessId(applicationProcessId);
		ServiceType = serviceType;
		ServiceSubtype = serviceSubtype;
	...
	public Telemetry(int applicationProcessId, byte serviceType, byte serviceSubtype, int sequenceCount)
	{
		setApplicationProcessId(applicationProcessId);
		ServiceType = serviceType;
		ServiceSubtype = serviceSubtype;
	...
	private int computeDfhOptionalFieldsLength()
	{
		...
		if(destinationIdPc.HasValue)
			optionalFieldsLength += ParameterConverter.GetStorageSize(destinationIdPc.Value);

		// Time
		ParameterCode? timePc;
		if(!settings.TimePcPerApid.TryGetValue(ApplicationProcessId, out timePc))
			timePc = settings.DefaultTimePc;
		if(timePc.HasValue)
			optionalFieldsLength += ParameterConverter.GetStorageSize(timePc.Value);
		...
		return checked((int)optionalFieldsLength);
	}
	...
	protected int WriteDataFieldHeaderToBuffer(byte[] buffer, int start)
	{
		...
		if(destinationIdPc.HasValue)
		{
			int insertedBits = ParameterConverter.InsertValue(buffer, index * 8, DestinationId, destinationIdPc.Value);
		...
		if(timePc.HasValue)
		{
			int insertedBits = ParameterConverter.InsertValue(buffer, index * 8, Time, timePc.Value);
			index += ParameterConverter.GetStorageSize(insertedBits);
		}
	...
	public static Telemetry FromBuffer(byte[] buffer)
	{
	...
	/// <returns>The read <see cref="Telemetry"/> packet.</returns>
	public static Telemetry FromBuffer(byte[] buffer, int start)
	{
		TelemetrySettings settings = EffectiveSettings;
		...
		unchecked
		{
			...
			s	throw new NotSupportedException("The telemetry packet contained in the buffer refers to an unsupported PUS version "+tmSourcePacketPusVersionNumber+", only 1 is supported." );
			...
			if(destinationIdPc.HasValue)
			{
				telemetry.DestinationId = ParameterConverter.ExtractValue<uint>(buffer, destinationIdPc.Value, index * 8); // offset is bytes, need bits!
				index += ParameterConverter.GetStorageSize(ParameterConverter.GetBitsSize(destinationIdPc.Value));
			}

			// Time
			ParameterCode? timePc;
			if(!settings.TimePcPerApid.TryGetValue(telemetry.ApplicationProcessId, out timePc))
				timePc = settings.DefaultTimePc;
			if(timePc.HasValue)
			{
				telemetry.Time = ParameterConverter.ExtractValue<TimeSpan>(buffer, timePc.Value, index * 8); // offset is bytes, need bits!
				index += ParameterConverter.GetStorageSize(ParameterConverter.GetBitsSize(timePc.Value)); ;
			}
	...
	/// <returns>The read <see cref="Telemetry"/> packet.</returns>
	public static Telemetry FromBuffer(byte[] buffer, int start, TelemetrySettings settings)
	{
