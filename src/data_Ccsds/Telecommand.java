...
public class Telecommand extends CcsdsPacket
{
	...
	/// relate only to acknowledgement of successful acceptance and execution,
	...
	public override boolean HasPacketErrorControlField
	{
		get { return true; } // PEC always present on TCs
	}
	...
	public override int PacketDataFieldAlignment
	{
		get
		{
			return EffectiveSettings.DataFieldPadding;
		}
	}
	...
	/// </remarks>
	public static TelecommandSettings GlobalSettings { get; set; }


	private static TelecommandSettings _telecommandSettings;
	...
	/// If you just want to get the effective settings, use <see cref="EffectiveSettings"/> instead.
	/// </remarks>
	public static TelecommandSettings Settings
	{
		get { return _telecommandSettings; }
		set { _telecommandSettings = value; }
	}
	...
	public Telecommand(int applicationProcessId, byte ack, byte serviceType, byte serviceSubtype)
	: this(applicationProcessId)
	{
		Ack = ack;
	...
	public Telecommand(int applicationProcessId, byte ack, byte serviceType, byte serviceSubtype, int sequenceCount)
	: this(applicationProcessId, ack, serviceType, serviceSubtype)
	{
	...
	protected override int ComputeDataFieldHeaderLength()
	{
	...
	private int computeDfhOptionalFieldsLength()
	{
		...
		if(sourceIdPc.HasValue)
			optionalFieldsLength += ParameterConverter.GetStorageSize(sourceIdPc.Value);
	...
	protected override int WriteDataFieldHeaderToBuffer(byte[] buffer, int start)
	{
		var settings = EffectiveSettings;
		...
		if(sourceIdPc.HasValue)
		{
			int insertedBits = ParameterConverter.InsertValue(buffer, index * 8, SourceId, sourceIdPc.Value);
	...
	public static new Telecommand FromBuffer(byte[] buffer)
	{
	...
	/// <returns>The read <see cref="Telecommand"/> packet.</returns>
	public static new Telecommand FromBuffer(byte[] buffer, int start)
	{
		var settings = EffectiveSettings;
		...
		unchecked
		{
			...
			if(tcPacketPusVersionNumber != 1)
				throw new NotSupportedException(string.Format("The telecommand packet contained in the buffer refers to an unsupported PUS version {0}, only 1 is supported.", tcPacketPusVersionNumber));
			...
			if(sourceIdPc.HasValue)
			{
				telecommand.SourceId = ParameterConverter.ExtractValue<uint>(buffer, sourceIdPc.Value, index * 8); // offset is bytes, need bits!
				index += ParameterConverter.GetStorageSize(ParameterConverter.GetBitsSize(sourceIdPc.Value));
			}
	...
	public static Telecommand FromBuffer(byte[] buffer, int start, TelecommandSettings settings)
	{
		...
		var oldSettings = Telecommand.Settings;
